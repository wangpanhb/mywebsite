// $(function(){
// 	var len=$(".music_main .music_wrap ul li").length;//获取轮播图的个数
// 	var index_2=0;//当前轮播图编号
// 	var timer=800;
// 	var intervaltimer=0;
// 	var isMoving=false;
...
// 		if (isMoving==false) {
// 			isMoving=true;
// 			var prev,next,hidden;//prev为选取前面的图的对象，next为后面选取的对象
// 			var curr=$("#imgCard"+index_2);//获取当前中间的图
// 			if (index_2==0) {
// 				prev=$("#imgCard"+(len-1));//第0张时候前一张图
// 			}else{
// 				prev=$("#imgCard"+(index_2-1));
// 			}
// 			if(index_2==(len-1)){//最后一张时候
// 				next=$("#imgCard0");
// 			}else{
// 				next=$("imgCard"+(index_2+1));//对象获取结束
// 			}
// 			if(slideMode){//当为真的时候执行上一张操作
// 				if (index_2-2>=0) {//从当前图算起相差两个的图隐藏
...
// 				$(".music_main .music_wrap ul li").find("p").css({"bottom":"-50px"});//所有标题隐藏
...
// 				//统一样式
// 				$(".music_main .music_wrap ul li").css({width:"300px",height:"120px",left:"600px",top:"60px",opacity:0});
...
// 				}
// 			}else{//当前比被点击大且相邻									//从0    跳到     4		要执行上一张方法
...
// 		});
// 		$(".music_main .music_wrap ul li").on("mouseover",function(){
// 			if($(this).css("width")=="670px"){//鼠标移入为当前正中显示的图片li，则清除定时器
// 				clearInterval(intervaltimer);
// 				intervaltimer=null;
// 			}
// 		}).on("mouseout",function(){//鼠标移出后重新滚动
// 			clearInterval(intervaltimer);
// 			intervaltimer=null;
